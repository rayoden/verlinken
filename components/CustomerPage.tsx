...
// Öffnungszeiten Top Job Bern (Mo–Fr 08–17, Sa 09–13, So geschlossen)
    ...
    <>
        ...
        @keyframes gold-shimmer {
          0% {
            box-shadow: 0 0 6px rgba(251, 191, 36, 0.4),
              0 0 12px rgba(245, 158, 11, 0.3), 0 0 18px rgba(217, 119, 6, 0.2);
          }
          50% {
            box-shadow: 0 0 10px rgba(251, 191, 36, 0.65),
              0 0 22px rgba(245, 158, 11, 0.45), 0 0 32px rgba(217, 119, 6, 0.3);
          }
          100% {
            box-shadow: 0 0 6px rgba(251, 191, 36, 0.4),
              0 0 12px rgba(245, 158, 11, 0.3), 0 0 18px rgba(217, 119, 6, 0.2);
      ...
      <main className="min-h-dvh bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white overflow-x-hidden">
        ...
        <div className="fixed inset-0 pointer-events-none overflow-hidden">
          <div
            className={`absolute -top-32 -right-32 w-96 h-96 ${theme.glow} rounded-full blur-3xl`}
          />
        ...
        <div className="relative z-10 max-w-md mx-auto px-5 pt-2.5 pb-3 md:pt-8 md:pb-6">
          ...
          <div className="mt-3 md:mt-5 space-y-2.5 md:space-y-3.5">
            {buttons.map((btn, i) => (
              ...
              >
                <div
                  className={`rounded-2xl border backdrop-blur-md px-4 py-3.5 md:px-5 md:py-4 flex items-center gap-3.5 md:gap-4 ${btn.bg} hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 shadow-lg ${
                    btn.label === "Google Bewertung" ? "gold-glow" : ""
                  }`}
                >
                  <div
                    className={`w-[48px] h-[48px] md:w-14 md:h-14 rounded-xl flex items-center justify-center shrink-0 ${btn.iconBg}`}
                  >
                    {btn.icon}
                  </div>

                  <div className="flex-1 min-w-0 text-left">
                    <p className="font-bold text-[16.5px] md:text-lg leading-tight">
                      {btn.label}
                    </p>
                    <p className="text-[13px] md:text-sm text-white/70 truncate">
                      {btn.sub}
                    </p>
                  </div>

                  <span className="text-lg md:text-xl opacity-50 group-hover:opacity-100 group-hover:translate-x-1 transition shrink-0">
                    →
                  </span>
                </div>
              </a>
          ...
          >
            <span>
              ...
              <a
                href="https://verlinken.ch"
                target="_blank"
                rel="noopener noreferrer"
                className="font-semibold hover:text-slate-300 transition"
              >
                verlinken.ch
              </a>
            </span>
            ...
            <span>
              Powered by <span className="font-semibold">Deno</span>
            </span>
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-slate-900 border border-white/10 rounded-3xl p-6 shadow-2xl">
              ...
              <div className="mt-6 flex gap-3">
                <a
                  href={`tel:${customer.phone}`}
                  className="flex-1 py-3 rounded-xl bg-white/10 text-center font-medium hover:bg-white/15 transition"
                >
                  Anrufen
                </a>
                <a
                  href={customer.whatsapp}
                  target="_blank"
                  className="flex-1 py-3 rounded-xl bg-emerald-600 text-center font-medium hover:bg-emerald-500 transition"
                >
                  WhatsApp
                </a>
              </div>
